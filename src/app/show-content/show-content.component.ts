...
export class ShowContentComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.communication.editToPreviewObserve.subscribe((res: string) => {
      this.editContent = res;
      console.log(res);
    });
    this.communication.refreshPreviewObserve.subscribe((res: boolean) => {
      if (res && this.editContent) {
  ...
  read() {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
      if (err) {
        ...
        this.communication.setContentToEdit(data);
        this.convertAndDraw();
      }
    });
